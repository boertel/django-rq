...
def get_redis_connection(config, use_strict_redis=False):
    ...
    if 'URL' in config:
        return redis_cls.from_url(config['URL'], db=config.get('DB'), ssl_cert_reqs=None)
    ...
    return redis_cls(host=config['HOST'], port=config['PORT'], db=config['DB'], password=config.get('PASSWORD'), ssl=config.get('SSL', False), ssl_cert_reqs=None)
